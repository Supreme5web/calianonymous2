...
async function initFirebase() {
  ...
  firebaseReadyPromise = new Promise(async (resolve) => {
    if (!isFirebaseConfigured()) {
      firebaseLastResult = { status: FirebaseStatus.disabled, reason: "Firebase config still has placeholders." };
      resolve(firebaseLastResult);
      return;
    }

    if (!window.firebase) {
      firebaseLastResult = { status: FirebaseStatus.error, reason: "Firebase CDN scripts did not load." };
      resolve(firebaseLastResult);
      return;
    }
    ...
    try {
      ...
      firebaseLastResult = { status: FirebaseStatus.ready, user: firebaseUser, app: firebaseApp, analytics: firebaseAnalytics };
      resolve(firebaseLastResult);
    } catch (error) {
      console.error("Firebase initialization failed:", error);
      firebaseLastResult = { status: FirebaseStatus.error, reason: formatFirebaseError(error), code: error.code || "" };
      resolve(firebaseLastResult);
    }
  });
...
window.CaliFirebase = {
  ...
  diagnostics() {
    return {
      status: this.status,
      lastResult: firebaseLastResult,
      configured: isFirebaseConfigured(),
      sdkLoaded: Boolean(window.firebase),
      authLoaded: Boolean(window.firebase?.auth),
      firestoreLoaded: Boolean(window.firebase?.firestore),
